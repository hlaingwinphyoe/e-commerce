...
@section('classes', 'admin admin-regions admin-regions-edit')

@section('content-header')
<x-admin.content-header :navs="['regions', 'edit']"></x-admin.content-header>
@endsection
            ...
            <div class="col-md-8 mb-2">
                <div class="py-3">
                    <div class="form-group">
                        ...
                        <input type="text" name="name" class="form-control form-control-sm" placeholder="Name" value="{{ old('name') ?? $region->name }}">
                        @error('name')
                     ...
                     <div class="form-group">
                        ...
                        <small class="help-text text-muted">အမည်ထည့်ပါ။ တူ၍မရပါ။</small>
                        <input type="text" name="mm_name" class="form-control form-control-sm" placeholder="MM Name" value="{{ old('mm_name') ?? $region->mm_name }}">
                        @error('mm_name')
                        <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="country" class="form-select">
                            ...
                            <option value="{{ $country->id }}" {{ $region->country && $region->country->id == $country->id ? 'selected' : '' }}>{{ $country->name }}</option>
                            @endforeach
                        </select>
                        @error('type')
                        <span class="text-danger">{{ $message }}</span>
