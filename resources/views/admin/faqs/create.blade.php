...
@section('classes', 'admin admin-faqs admin-faqs-create')

@section('content-header')
<x-admin.content-header :navs="['faqs', 'create']"></x-admin.content-header>
@endsection
            ...
            <div class="col-md-8 mb-2">
                ...
                <div class="form-group">
                    ...
                    <select name="faq_type" class="form-select">
                        ...
                        <option value="{{ $type->id }}" {{ old('faq_type') == $type->id ? 'selected' : '' }}>{{ $type->name }}</option>
                        @endforeach
                    </select>
                    @error('title')
                    <span class="text-danger">{{ $message }}</span>
                ...
                <div class="form-group">
                    ...
                    <textarea name="description" class="text-editor form-control form-control-sm" rows="3" placeholder="Description">{{ old('description') }}</textarea>
                    @error('description')
                    <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
